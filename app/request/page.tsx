...
export default function RequestPage() {
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      const paymentReference = `FREE-${timestamp}`
      const serialNumber = `FREE-${timestamp}`

      // Create certificate directly with amount: 0
      ...
      // Get the created certificate from the most recent entry
      if (certificates.length > 0) {
        const createdCert = certificates[0]
        // Redirect to certificate details page
        router.push(`/certificate/${createdCert.id}`)
      } else {
    ...
    <div className="flex min-h-screen flex-col bg-[#F9FAFB]">
      ...
      <div className="fixed bottom-0 left-0 right-0 border-t border-[#E5E7EB] bg-white p-4 pb-[max(16px,env(safe-area-inset-bottom))]">
        <Button
          ...
          disabled={isLoading}
          className="h-14 w-full rounded-xl bg-[#10B981] text-base font-semibold text-white hover:bg-[#059669] disabled:bg-gray-400 disabled:cursor-not-allowed"
        >
          {isLoading ? "Submitting..." : "Submit Request (FREE)"}
        </Button>
      </div>
    </div>
  )
